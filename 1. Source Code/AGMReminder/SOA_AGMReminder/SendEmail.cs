...
namespace SOA_AGMReminder
{
    public class SendEmail
    {
        ...
        string sLateFilingDate = ConfigurationManager.AppSettings["LateFilingDate"];
        string sLateFilingCost = ConfigurationManager.AppSettings["LateFilingCost"];
        ...
        public DataSet GetCardCode()
        {
            ...
            try
            {
                ...
                sProcName = "AB_AGM_SP001_GetCardCodes";
        ...
        public DataSet CheckDuplicateMailSending(string sCode, string sName, string sAGMDate, string Mail)
        {
            string sFuncName = string.Empty;
            DataSet oDataset = new DataSet();
            string sProcName = string.Empty;
            try
            {
                ...
                sProcName = "AB_AGM_SP002_CheckDuplicateMailSending";
                oLog.WriteToDebugLogFile("Starting Function ", sFuncName);
                ...
                oDataset = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, sProcName, Data.CreateParameter("@CardCode", sCode),
                    Data.CreateParameter("@CardName", sName), Data.CreateParameter("@AGMDate", sAGMDate), Data.CreateParameter("@Email", Mail));
        ...
        public string ComposeBody(string sContPrsnName, string sCompanyName, int monthsToAdd)
        {
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                else if (monthsToAdd == 5)
                {
                    ...
                    using (var sReader = new StreamReader(sTemplatePath))
                    {
                        ...
                        sbMail.Replace("{ContactPersonName}", System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(sContPrsnName.ToLower()));
                        sbMail.Replace("{CompanyName}", System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(sCompanyName.ToLower()));
                        sbMail.Replace("{LateFilingDate}", sLateFilingDate);
                        sbMail.Replace("{LateFilingCost}", sLateFilingCost);
                    }
                }
        ...
        public string InsertEmailLog(string sCode, string sName, string sAGMDate, string sDateSent, string Mail)
        {
            ...
            try
            {
                ...
                //string sInsertQuery = "insert into [@EMAILLOG_SOA](" + sColumnNames + ") values(" + sParameter + ")";
                sProcName = "AB_AGM_SP003_InsertAGMLog";
                oLog.WriteToDebugLogFile("Starting Function ", sFuncName);

                oLog.WriteToDebugLogFile("Calling Run_StoredProcedure() " + sProcName, sFuncName);
                oDataset = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, sProcName, Data.CreateParameter("@CardCode", sCode),
                    Data.CreateParameter("@CardName", sName), Data.CreateParameter("@AGMDate", sAGMDate), Data.CreateParameter("@DateSent", sDateSent),
        ...
        public string SendAutomatedEmail(string sEmailTo, string sCntctPrsnName, string sCompanyName, int monthsToAdd, ref string sErrDesc)
        {
            ...
            try
            {
                ...
                MailMessage message = CreateDefaultMailMessage(sFromMailId, sEmailTo, sCntctPrsnName, sCompanyName, monthsToAdd, ref sErrDesc);
        ...
        public string ComposeErrorBody(DataTable dtBody)
        {
            ...
            foreach (DataRow item in dtBody.Rows)
            {
                sBodyDetail = "<tr><td>&nbsp;" + item["CardCode"] + "</td><td>&nbsp;" + item["CardName"].ToString() + " </td> " +
                    " <td>&nbsp;" + item["AGMDate"].ToString() + " </td><td>&nbsp;" + item["Email"].ToString() + " </td> " +
            ...
            sTableFormat = "<table border = '1' cellspacing = 0 cellpadding = 0 style='font-size:10.0pt;font-family:Arial;width: 85%;'> " +
                                ...
                                "<td><strong style='color: blue; background-color: transparent;'>&nbsp;AGM Date &nbsp;</strong></td> " +
            ...
            string stextBody = "<p style='font-size:10.0pt;font-family:Arial;'>Dear Admin,<br /><br /> Below is a list of the Customers that have not been Send out the AGM and Annual Return Filing Reminder. Please check. " +
        ...
        private MailMessage CreateDefaultMailMessage(string MailFrom, string MailTo, string sCntctPrsnName, string sCompanyName, int monthsToAdd, ref string sErrDesc)
        {
            ...
            try
            {
                ...
                message.Body = ComposeBody(sCntctPrsnName, sCompanyName, monthsToAdd);
                message.IsBodyHtml = true;
