...
namespace SOA_AGMReminder
{
    class Program
    {
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow dr in ds.Tables[0].Rows)
                        {
                            ...
                            if (DateTime.Now.Date <= Convert.ToDateTime(sFinancialYearEnd).AddMonths(3).AddDays(1).Date)
                            {
                                monthsToAdd = 3;
                            }
                            else if (DateTime.Now.Date > Convert.ToDateTime(sFinancialYearEnd).AddMonths(3).AddDays(1).Date && DateTime.Now.Date <= Convert.ToDateTime(sFinancialYearEnd).AddMonths(4).AddDays(1).Date)
                            {
                                monthsToAdd = 4;
                            }
                            else if (DateTime.Now.Date >= Convert.ToDateTime(sFinancialYearEnd).AddMonths(5).AddDays(1).Date)
                            {
                                monthsToAdd = 5;
                            }

                            string sAGMDate = Convert.ToDateTime(sFinancialYearEnd).AddMonths(monthsToAdd).AddDays(1).ToString("MM/dd/yyyy");
                            string sSendingEmailDate = DateTime.Now.Date.ToString("MM/dd/yyyy");

                            if (sAGMDate == sSendingEmailDate)
                            {
                                //Check to avoid duplicate mail sending
                                DataSet dsDuplicateCheck = oSendMail.CheckDuplicateMailSending(dr["Code"].ToString(), dr["Name"].ToString(), sAGMDate, dr["Mail"].ToString());
                                ...
                                if (dsDuplicateCheck.Tables[0].Rows[0]["Status"].ToString() == "Yes")
                                {
                                    ...
                                    string lReturn = oSendMail.SendAutomatedEmail(dr["Mail"].ToString(), dr["Name"].ToString(), dr["CompanyName"].ToString(), monthsToAdd, ref sErrDesc);
                                    if (lReturn == "SUCCESS")
                                    {
                                        ...
                                        string sResult = oSendMail.InsertEmailLog(dr["Code"].ToString(), dr["Name"].ToString(), sAGMDate, DateTime.Now.ToString(), dr["Mail"].ToString());
                                    ...
                                    else
                                    {
                                        ...
                                        dt.Rows.Add(dr["Code"].ToString(), dr["Name"].ToString(), sAGMDate, dr["Mail"].ToString(), lReturn.ToString());
                                    }
                                }
                                else
                                {
                            ...
                            {
                                oLog.WriteToDebugLogFile("Current date " + sSendingEmailDate + " doesn't match the reminder date " + sAGMDate + " to send email", sFuncName);
                                Console.WriteLine("Current date " + sSendingEmailDate + " doesn't match the reminder date " + sAGMDate + " to send email");
                            }
                        }
                    }
                }
        ...
        static DataTable ErrorTable()
        {
            ...
            table.Columns.Add("AGMDate", typeof(string));
