...
namespace PersonalTaxFilingReminder
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            string sPersonalTaxFilingDate = string.Empty;
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (DateTime.Now.Date <= Convert.ToDateTime(s1stReminderDate).Date)
                        {
                            ...
                            sPersonalTaxFilingDate = s1stReminderDate;
                            sSuffix = "st";
                        }
                        else if (DateTime.Now.Date > Convert.ToDateTime(s1stReminderDate).Date && DateTime.Now.Date <= Convert.ToDateTime(s2ndReminderDate).Date)
                        {
                            ...
                            sPersonalTaxFilingDate = s2ndReminderDate;
                            sSuffix = "nd";
                        }

                        string sSendingEmailDate = DateTime.Now.Date.ToString("MM/dd/yyyy");

                        if (sPersonalTaxFilingDate == sSendingEmailDate)
                        {
                            foreach (DataRow dr in ds.Tables[0].Rows)
                            {
                                //Check to avoid duplicate mail sending
                                DataSet dsDuplicateCheck = oSendMail.CheckDuplicateMailSending(dr["Code"].ToString(), dr["Name"].ToString(), sPersonalTaxFilingDate, dr["Mail"].ToString());
                                ...
                                if (dsDuplicateCheck.Tables[0].Rows[0]["Status"].ToString() == "Yes")
                                {
                                    ...
                                    string lReturn = oSendMail.SendAutomatedEmail(dr["Mail"].ToString(), dr["Name"].ToString(), dr["CompanyName"].ToString(), ToIdentifyTemplate, sSuffix, ref sErrDesc);
                                    if (lReturn == "SUCCESS")
                                    {
                                        ...
                                        string sResult = oSendMail.InsertEmailLog(dr["Code"].ToString(), dr["Name"].ToString(), sPersonalTaxFilingDate, DateTime.Now.ToString(), dr["Mail"].ToString());
                                    ...
                                    else
                                    {
                                        ...
                                        dt.Rows.Add(dr["Code"].ToString(), dr["Name"].ToString(), sPersonalTaxFilingDate, dr["Mail"].ToString(), lReturn.ToString());
                                    }
                                }
                                else
                                {
                        ...
                        {
                            oLog.WriteToDebugLogFile("Current date " + sSendingEmailDate + " doesn't match the reminder date " + sPersonalTaxFilingDate + " to send email", sFuncName);
                            Console.WriteLine("Current date " + sSendingEmailDate + " doesn't match the reminder date " + sPersonalTaxFilingDate + " to send email");
                        }
                    }
                }
        ...
        static DataTable ErrorTable()
        {
            ...
            table.Columns.Add("PTAXFILINGDate", typeof(string));
