...
namespace SOA_ECIFilingReminder
{
    public class SendEmail
    {
        ...
        string sFilingDueDate = ConfigurationManager.AppSettings["FilingDueDate"];
        ...
        public DataSet GetCardCode()
        {
            ...
            try
            {
                ...
                sProcName = "AB_ECI_SP001_GetCardCodes";
        ...
        public DataSet CheckDuplicateMailSending(string sCode, string sName, string sECIDate, string Mail)
        {
            string sFuncName = string.Empty;
            DataSet oDataset = new DataSet();
            string sProcName = string.Empty;
            try
            {
                ...
                sProcName = "AB_ECI_SP002_CheckDuplicateMailSending";
                oLog.WriteToDebugLogFile("Starting Function ", sFuncName);
                ...
                oDataset = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, sProcName, Data.CreateParameter("@CardCode", sCode),
                    Data.CreateParameter("@CardName", sName), Data.CreateParameter("@ECIDate", sECIDate), Data.CreateParameter("@Email", Mail));
        ...
        public string ComposeBody(string sContPrsnName, string sCompanyName, int ToIdentifyTemplate, string sFinancialYear)
        {
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (ToIdentifyTemplate == 1)
                {
                    ...
                    using (var sReader = new StreamReader(sTemplatePath))
                    {
                        ...
                        sbMail.Replace("{FilingDueDate}", sFilingDueDate);
                        sbMail.Replace("{YearEnd}", sFinancialYear);
                    }
                }
                else if (ToIdentifyTemplate == 2)
                {
                    ...
                    using (var sReader = new StreamReader(sTemplatePath))
                    {
                        ...
                        sbMail.Replace("{CompanyName}", System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(sCompanyName.ToLower()));
                        sbMail.Replace("{FilingDueDate}", sFilingDueDate);
                        sbMail.Replace("{Reminder1Date}", Convert.ToDateTime(sFinancialYear).AddDays(10).ToString("MM/dd/yyyy"));
        ...
        public string InsertEmailLog(string sCode, string sName, string sECIDate, string sDateSent, string Mail)
        {
            ...
            try
            {
                ...
                //string sInsertQuery = "insert into [@EMAILLOG_SOA](" + sColumnNames + ") values(" + sParameter + ")";
                sProcName = "AB_ECI_SP003_InsertECILog";
                oLog.WriteToDebugLogFile("Starting Function ", sFuncName);

                oLog.WriteToDebugLogFile("Calling Run_StoredProcedure() " + sProcName, sFuncName);
                oDataset = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, sProcName, Data.CreateParameter("@CardCode", sCode),
                    Data.CreateParameter("@CardName", sName), Data.CreateParameter("@ECIDate", sECIDate), Data.CreateParameter("@DateSent", sDateSent),
        ...
        public string SendAutomatedEmail(string sEmailTo, string sCntctPrsnName, string sCompanyName, int ToIdentifyTemplate, string sSuffix, string sFinancialYear, ref string sErrDesc)
        {
            ...
            try
            {
                ...
                MailMessage message = CreateDefaultMailMessage(sFromMailId, sEmailTo, sCntctPrsnName, sCompanyName, ToIdentifyTemplate, sSuffix, sFinancialYear, ref sErrDesc);
        ...
        public string ComposeErrorBody(DataTable dtBody)
        {
            ...
            foreach (DataRow item in dtBody.Rows)
            {
                sBodyDetail = "<tr><td>&nbsp;" + item["CardCode"] + "</td><td>&nbsp;" + item["CardName"].ToString() + " </td> " +
                    " <td>&nbsp;" + item["ECIDate"].ToString() + " </td><td>&nbsp;" + item["Email"].ToString() + " </td> " +
            ...
            sTableFormat = "<table border = '1' cellspacing = 0 cellpadding = 0 style='font-size:10.0pt;font-family:Arial;width: 85%;'> " +
                                ...
                                "<td><strong style='color: blue; background-color: transparent;'>&nbsp;ECI Date &nbsp;</strong></td> " +
            ...
            string stextBody = "<p style='font-size:10.0pt;font-family:Arial;'>Dear Admin,<br /><br /> Below is a list of the Customers that have not been Send out the ECI Filing Reminder. Please check. " +
        ...
        private MailMessage CreateDefaultMailMessage(string MailFrom, string MailTo, string sCntctPrsnName, string sCompanyName, int ToIdentifyTemplate, string sSuffix, string sFinancialYear, ref string sErrDesc)
        {
            ...
            try
            {
                ...
                message.Body = ComposeBody(sCntctPrsnName, sCompanyName, ToIdentifyTemplate, sFinancialYear);
                message.IsBodyHtml = true;
