...
namespace SOA_TaxFilingReminder
{
    public class SendEmail
    {
        ...
        string sLastFilingDate = ConfigurationManager.AppSettings["LastFilingDate"];
        ...
        public DataSet GetCardCode()
        {
            ...
            try
            {
                ...
                sProcName = "AB_TAXFILING_SP001_GetCardCodes";
        ...
        public DataSet CheckDuplicateMailSending(string sCode, string sName, string sTAXFILINGDate, string Mail)
        {
            string sFuncName = string.Empty;
            DataSet oDataset = new DataSet();
            string sProcName = string.Empty;
            try
            {
                ...
                sProcName = "AB_TAXFILING_SP002_CheckDuplicateMailSending";
                oLog.WriteToDebugLogFile("Starting Function ", sFuncName);
                ...
                oDataset = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, sProcName, Data.CreateParameter("@CardCode", sCode),
                    Data.CreateParameter("@CardName", sName), Data.CreateParameter("@TAXFILINGDate", sTAXFILINGDate), Data.CreateParameter("@Email", Mail));
        ...
        public string ComposeBody(string sContPrsnName, string sCompanyName, int ToIdentifyTemplate, string s1stReminderDate, string sFinancialYear)
        {
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (ToIdentifyTemplate == 1)
                {
                    ...
                    using (var sReader = new StreamReader(sTemplatePath))
                    {
                        ...
                        sbMail.Replace("{FilingDueYear}", DateTime.Now.Year.ToString());
                        sbMail.Replace("{FinancialYearEnd}", sFinancialYear);
                    }
                }
                else if (ToIdentifyTemplate == 2)
                {
                    ...
                    using (var sReader = new StreamReader(sTemplatePath))
                    {
                        ...
                        sbMail.Replace("{CompanyName}", System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(sCompanyName.ToLower()));
                        sbMail.Replace("{FilingDueYear}", DateTime.Now.Year.ToString());
                        sbMail.Replace("{Reminder1Date}", s1stReminderDate);
                    }
                }
                else if (ToIdentifyTemplate == 3)
                {
                    string sTemplatePath = AppDomain.CurrentDomain.BaseDirectory;
                    int index = sTemplatePath.IndexOf("\\bin");
                    if (index > 0)
                        sTemplatePath = sTemplatePath.Substring(0, index) + "\\Email Template\\Reminder3.htm";
                    ...
                    using (var sReader = new StreamReader(sTemplatePath))
                    {
                        ...
                        sbMail.Replace("{CompanyName}", System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(sCompanyName.ToLower()));
                        sbMail.Replace("{FilingDueYear}", DateTime.Now.Year.ToString());
                        sbMail.Replace("{LastFilingDate}", sLastFilingDate);
                    }
                }
        ...
        public string InsertEmailLog(string sCode, string sName, string sTAXFILINGDate, string sDateSent, string Mail)
        {
            ...
            try
            {
                ...
                //string sInsertQuery = "insert into [@EMAILLOG_SOA](" + sColumnNames + ") values(" + sParameter + ")";
                sProcName = "AB_TAXFILING_SP003_InsertTAXFILINGLog";
                oLog.WriteToDebugLogFile("Starting Function ", sFuncName);

                oLog.WriteToDebugLogFile("Calling Run_StoredProcedure() " + sProcName, sFuncName);
                oDataset = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, sProcName, Data.CreateParameter("@CardCode", sCode),
                    Data.CreateParameter("@CardName", sName), Data.CreateParameter("@TAXFILINGDate", sTAXFILINGDate), Data.CreateParameter("@DateSent", sDateSent),
        ...
        public string SendAutomatedEmail(string sEmailTo, string sCntctPrsnName, string sCompanyName, int ToIdentifyTemplate, string s1stReminderDate, string sSuffix, string sFinancialYear, ref string sErrDesc)
        {
            ...
            try
            {
                ...
                MailMessage message = CreateDefaultMailMessage(sFromMailId, sEmailTo, sCntctPrsnName, sCompanyName, ToIdentifyTemplate, s1stReminderDate, sSuffix, sFinancialYear, ref sErrDesc);
        ...
        public string ComposeErrorBody(DataTable dtBody)
        {
            ...
            foreach (DataRow item in dtBody.Rows)
            {
                sBodyDetail = "<tr><td>&nbsp;" + item["CardCode"] + "</td><td>&nbsp;" + item["CardName"].ToString() + " </td> " +
                    " <td>&nbsp;" + item["TAXFILINGDate"].ToString() + " </td><td>&nbsp;" + item["Email"].ToString() + " </td> " +
            ...
            sTableFormat = "<table border = '1' cellspacing = 0 cellpadding = 0 style='font-size:10.0pt;font-family:Arial;width: 85%;'> " +
                                ...
                                "<td><strong style='color: blue; background-color: transparent;'>&nbsp;Tax Filing Date &nbsp;</strong></td> " +
            ...
            string stextBody = "<p style='font-size:10.0pt;font-family:Arial;'>Dear Admin,<br /><br /> Below is a list of the Customers that have not been Send out the Tax Filing Reminder. Please check. " +
        ...
        private MailMessage CreateDefaultMailMessage(string MailFrom, string MailTo, string sCntctPrsnName, string sCompanyName, int ToIdentifyTemplate, string s1stReminderDate, string sSuffix, string sFinancialYear, ref string sErrDesc)
        {
            ...
            try
            {
                ...
                message.Body = ComposeBody(sCntctPrsnName, sCompanyName, ToIdentifyTemplate, s1stReminderDate, sFinancialYear);
                message.IsBodyHtml = true;
